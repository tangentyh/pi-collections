...
| [pi-sticky-last-prompt](extensions/sticky-last-prompt) | Pins the latest user message that has scrolled above the viewport top as a one-line bar at the top of the fullscreen TUI (the bar hides while every prompt is still on screen; older prompts take over as they leave the view); left-clicking the bar scrolls the transcript to that message |
